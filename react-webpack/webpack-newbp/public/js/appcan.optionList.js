...
var model_hiddenLine =  '<li style="position: relative;border-bottom:1px solid #BABABA;color: black;display: -webkit-box !important;display: box !important;-webkit-box-align: center;box-align: center; padding:0;"  <%if(group){%>data-group="<%=group%>"<%}%> <%if(data.id){%>id="<%=data.id%>"<%}%> data-index="<%=index%>">\
...
<div class="scroll-right" data-index="<%=index%>" style="position: absolute;top: 1px;right: 0; color: #FFF;display: -webkit-box !important;display: box !important;-webkit-box-align: center;box-align: center;<%if(data.height){%>height:<%=data.height%>;line-height:<%=data.height%>;<%}else{%>height:98%;line-height:98%;<%}%>;background:<%=data.hideOption.style.background%>;"><div style="clear:both;font-size:<%=data.hideOption.style.fontSize%>;">\
