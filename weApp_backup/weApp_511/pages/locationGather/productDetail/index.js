...
Page({
  ...
  upLoadPhotoRecords: function () {
    ...
    //保存文件到本地。应该是上传保存在本地的文件的url
    var that = this;
    var temp = that.data.photoRecordList;
    var toULRecordList = [];
    temp.map(function (elem, index) {
      if (!toULRecordList[index]) {
        toULRecordList[index] = {};
        toULRecordList[index].CreateTime = temp[index].CreateTime;
        toULRecordList[index].Desc = temp[index].Desc;
        toULRecordList[index].UserNo = temp[index].UserNo;
      }
      elem.files.map(function (elem_sub, index_sub) {
        wx.saveFile({
          tempFilePath: elem_sub,
          success: function (res) {
            if (!toULRecordList[index].files) {
              toULRecordList[index].files = [];
            }
            toULRecordList[index].files[index_sub] = res.savedFilePath
          }
        });
      });
    });
    that.setData({
      toULRecordList: toULRecordList
    });
    console.log("图片保存到本地,最终的待上传的拍照收集记录,路径保存的是本地文件的路径", toULRecordList);
    //上传到服务器操作
    // wx.request({
    //   url: config.host + '****',
    //   method: 'GET',
    ...
    //     wx.hideToast();
    //     try {
    //       wx.removeStorageSync(that.data.projectNo + that.data.pointCode + that.data.productCode + 'photo');
    //     } catch (e) {
    //       wx.showToast({
    //         title: '上传成功,移出缓存失败,请联系管理员!',
    //         image: '/images/warn.png',
    //         duration: 2000
    //       });
    //     }
